...
Unless the `-DzkHost` or `-Dsolr.zookeeper.server.enabled=true` are specified at startup time, this section is ignored.
...
[%autowidth,frame=none]
|===
|Optional |Default: none (disabled)
|===
+
Enables a node-wide, segment-level Lucene query cache shared by all cores, and sets the maximum amount of heap it may use.
The value is either a percentage of the maximum JVM heap (e.g., `10%`), a value with a unit suffix (e.g., `512k`, `512m`, `1g`), or a plain number of bytes.
An empty value or `0` (the default) disables the cache.
+
Unlike the per-core xref:caches-warming.adoc[`filterCache`], which caches results for the whole index and must be re-warmed after every commit, this cache stores the results of frequently used non-scoring queries (and sub-queries) per segment, so entries for unchanged segments remain valid across commits with no warm-up.
Frequently used filter queries may be held in both caches, so consider filterCache sizing when enabling it.
+
In default configurations this property uses the value of the `solr.queryCache.maxRam` system property, e.g., `-Dsolr.queryCache.maxRam=10%`.
...
|===
+
The maximum number of queries held by the node-wide Lucene query cache.
Only used when <<queryCacheMaxRam,`queryCacheMaxRam`>> enables the cache.
In default configurations this property uses the value of the `solr.queryCache.count` system property.
...
This section is ignored unless the Solr instance is started with either `-Dsolr.zookeeper.server.enabled=TRUE` or `-DzkHost`
...
If no port is passed via the `solr.xml` (i.e., `0`), then Solr will default to the port that jetty is listening on, defined by `${solr.port.listen}`.
...
+
It is recommended to set this value to be larger than any xref:query-guide:common-query-parameters.adoc#timeallowed-parameter[`timeAllowed`] query parameter used to allow  xref:query-guide:common-query-parameters.adoc#timeallowed-parameter[`timeAllowed`]  to gracefully finish the request processing and return partial results before the coordinator gives up on the request.
...
|===
+
The URL scheme to be used in distributed search and other inter-node communication.
Note: this is obsoleted by the `solr.ssl.enabled` boolean system property.
