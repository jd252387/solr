...
The xref:highlighting.adoc#hl-matchedqueries[`hl.matchedQueries`] parameter of the unified highlighter relates each highlight to the named queries whose terms produced it, embedding the query name, the original (pre-analysis) query string, and the post-analysis term or phrase in the highlight's pre-tag.
